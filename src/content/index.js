...
function initMiniWindow() {
  ...
  document.querySelectorAll("audio.simple-tts-audio").forEach(audio => {
    audio.pause();
    audio.src = "";
    audio.remove();
  });
...
async function handleControlClick(e) {
  ...
  if (state === "paused" && player.isPaused()) {
    try {
      ...
      console.error("[Simple TTS] Resume failed:", err);
      setState("error");
    }
    return;
  }
...
async function playFromRequest(request) {
  ...
  try {
    ...
    console.error("[Simple TTS] Playback failed:", err);
    setState("error");
  } finally {
...
async function replayFromCache() {
  ...
  try {
    ...
    console.error("[Simple TTS] Replay failed:", err);
    setState("error");
  }
}
